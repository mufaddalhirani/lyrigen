...
import { Home, greeting } from './views/Home'
...
import { LyricsHub, type SyncRequest } from './views/LyricsHub'
import { LyricsNavBadge } from './components/AiLyricSync'
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // One rescan after a burst of finished downloads, not one per song: a
    // playlist finishing two dozen at a time rescanned the library two dozen times.
    let rescanTimer = 0
    const offJob = window.electronAPI.onDownloadJob(job => { if (job.status === 'done') { flash(`Downloaded ${job.metadata?.title || job.info?.title || 'a song'}${job.lyricPath ? ' with lyrics' : ''}.`); window.clearTimeout(rescanTimer); rescanTimer = window.setTimeout(() => { void window.electronAPI.rescanLibrary().then(() => refresh()) }, 2500) } void window.electronAPI.listDownloads().then(jobs => setActiveDownloads(active(jobs))).catch(() => undefined) })
    ...
    return () => { window.clearTimeout(rescanTimer); offJobs(); offJob(); offInbox() }
  }, [])
  ...
  useEffect(() => {
    ...
    void loadAppSettings().then(preferences => {
  ...
  // Songs added with "Play next" sit at the front of the queue, in the order they were added.
  const manualFront = (state: QueueState) => { const manual = new Set(state.manualTrackIds ?? []); let count = 0; while (count < state.upcomingTrackIds.length && manual.has(state.upcomingTrackIds[count])) count++; return count }
  const shuffled = (ids: string[]) => { const next = [...ids]; for (let i = next.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [next[i], next[j]] = [next[j], next[i]] } return next }
  // The order the queue had before shuffle, so turning shuffle off puts it back.
  const unshuffledRef = useRef<string[]>([])
  const setPlaybackModes = (shuffle: boolean, repeat: QueueState['repeat']) => {
    if (shuffle === queueState.shuffle) { void updateQueue({ ...queueState, repeat }); return }
    const front = manualFront(queueState)
    const head = queueState.upcomingTrackIds.slice(0, front)
    const rest = queueState.upcomingTrackIds.slice(front)
    if (shuffle) { unshuffledRef.current = rest; void updateQueue({ ...queueState, shuffle, repeat, upcomingTrackIds: [...head, ...shuffled(rest)] }) }
    else {
      const rank = new Map(unshuffledRef.current.map((id, index) => [id, index]))
      const restored = [...rest].sort((a, b) => (rank.get(a) ?? Infinity) - (rank.get(b) ?? Infinity))
      void updateQueue({ ...queueState, shuffle, repeat, upcomingTrackIds: [...head, ...restored] })
    }
  }
  ...
  // Typing updates the box at once; the 5,000-song filter follows a beat later.
  ...
  const filteredTracks = useMemo(() => {
    ...
    return tracks
  }, [activePlaylistId, deferredQuery, genreFilter, playlists, searchText, sort, sortedLibrary, trackById, view])
  const genres = useMemo(() => Array.from(new Set(library.map(track => track.genre).filter((genre): genre is string => Boolean(genre)))).sort(), [library]); const albums = useMemo(() => Array.from(new Map(library.map(track => [`${track.album}|${track.artist}`, track])).values()).sort((a, b) => collator.compare(a.album, b.album)), [library]); const artists = useMemo(() => Array.from(new Set(library.map(track => track.artist || 'Unknown artist'))).sort(collator.compare), [library])
  const playTrack = async (track: LibraryTrack, source = filteredTracks) => {
    ...
    // On shuffle, the rest of the collection (before and after the song) plays in a random order.
    const before = source.slice(0, Math.max(0, index)).map(item => item.id)
    if (queueState.shuffle) unshuffledRef.current = remaining
    await updateQueue({ ...queueState, currentTrackId: track.id, upcomingTrackIds: queueState.shuffle ? shuffled([...remaining, ...before]) : remaining, manualTrackIds: [], historyTrackIds: queueState.shuffle ? [] : before })
    setPlayerMode('full')
  }
  const playNext = async (track: LibraryTrack) => {
    if (!currentTrack) return playTrack(track)
    const others = queueState.upcomingTrackIds.filter(id => id !== track.id && id !== currentTrack.id)
    const manual = (queueState.manualTrackIds ?? []).filter(id => id !== track.id)
    const front = manualFront({ ...queueState, upcomingTrackIds: others, manualTrackIds: manual })
    await updateQueue({ ...queueState, upcomingTrackIds: [...others.slice(0, front), track.id, ...others.slice(front)], manualTrackIds: [...manual, track.id] })
    flash(`${track.title} will play next.`)
  }
  const playLast = async (track: LibraryTrack) => { if (!currentTrack) return playTrack(track); await updateQueue({ ...queueState, upcomingTrackIds: [...queueState.upcomingTrackIds.filter(id => id !== track.id && id !== currentTrack.id), track.id], manualTrackIds: (queueState.manualTrackIds ?? []).filter(id => id !== track.id) }); flash(`${track.title} added to the end of the queue.`) }
  ...
  const goNext = () => {
    // The queue is already in play order (shuffle shuffles it up front), so
    // the next song is always the first one waiting — the one shown as next.
    if (queueTracks[1]) {
      const target = queueTracks[1]
      void updateQueue({ ...queueState, currentTrackId: target.id, upcomingTrackIds: queueState.upcomingTrackIds.filter(id => id !== target.id), manualTrackIds: (queueState.manualTrackIds ?? []).filter(id => id !== target.id), historyTrackIds: [...queueState.historyTrackIds, currentTrack!.id].slice(-1000) })
    } else if (queueState.repeat === 'all' && currentTrack) {
      const ids = [...new Set([...queueState.historyTrackIds, currentTrack.id])].filter(id => trackById.has(id))
      const order = queueState.shuffle ? shuffled(ids) : ids
      if (order.length > 1) void updateQueue({ ...queueState, currentTrackId: order[0], upcomingTrackIds: order.slice(1), historyTrackIds: [] })
    }
  }; const goPrevious = () => { const previousId = queueState.historyTrackIds.at(-1); const previous = previousId ? trackById.get(previousId) : undefined; if (previous && currentTrack) void updateQueue({ ...queueState, currentTrackId: previous.id, upcomingTrackIds: [currentTrack.id, ...queueState.upcomingTrackIds], historyTrackIds: queueState.historyTrackIds.slice(0, -1) }) }
