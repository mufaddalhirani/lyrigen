    ...
    <div className="tool-columns" style={showSettings ? undefined : { gridTemplateColumns: '1fr' }}>
      ...
      {showSettings && settings && <aside className="tool-panel">
        ...
        <div className="tool-panel-body settings-grid">
          ...
          <label><span>Parallel downloads</span><select value={settings.concurrency} onChange={event => patchSettings({ concurrency: Number(event.target.value) })}>{[1, 2, 3, 4, 5, 6, 8].map(value => <option key={value} value={value}>{value}</option>)}</select></label>
